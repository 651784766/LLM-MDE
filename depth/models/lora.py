...
from depth.models.base_vit import ViT
...
class LoRA_ViT_timm(nn.Module):
    def __init__(self, vit_model: timm_ViT, r: int, alpha: int, num_classes: int = 0, lora_layer=None):
        ...
        for t_layer_i, blk in enumerate(vit_model.blocks):
            ...
            w_a_linear_q = nn.Linear(self.dim, r, bias=False)  # dim=768
...
if __name__ == "__main__":  # Debug
    ...
    lora_vit = LoRA_ViT_timm(vit_model=model, r=4, num_classes=10, alpha=1)
    pred = lora_vit(img)
    print(pred.shape)
